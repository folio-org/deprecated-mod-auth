...
function handle_things(req, res) {
    ...
    if(req.method == "GET") {
        ...
        if(get_thing(new_thing.name) != null) {
            res.status(400).json({"error":"A thing by that name already exists"});
        } else {
            var added_thing = add_thing(new_thing);
            if(added_thing != null) {
                res.status(201).json(added_thing);
            } else {
                res.status(400).json({"error" : "Invalid thing to add" });
            }
        }
    } else {
...
function handle_one_thing(req, res) {
    ...
    if(req.method == "GET") {
        ...
        if(thing == null) {
            res.status(404).json({ "error" : "No thing by the name: '" + name + "' could be found" });
        } else {
            res.status(200);
            if(see_sensitive) {
                res.json(thing);
